...
/*   Updated: 2023/09/14 21:42:33 by jaehulee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	init_mutex_fork(int argc, t_table **table)
{
	int	i;

	if (argc == 6 && (*table)->args->must_eat_count == 0)
		return (1);
...
static int	init_philos(t_table **table)
{
	...
	while (i < ((*table)->args->philo_count))
	{
		...
		(*table)->philos[i]->right_fork = (i + 1) % (*table)->args->philo_count;
...
static int	init_mutex_var(int argc, t_table **table)
{
	int	i;

	i = 0;
	if (init_mutex_fork(argc, table))
		return (1);
